...
<font face="黑体" size=4> 
**写在前面**: 数字签名算法(DSA)是原始的Elgamal数字签名算法的变体，Elgamal数字签名算法是基于离散对数公钥密码体系的算法。本文旨在不引入`群论`的情况下，在离散对数相关概念(阶，原根)的基础上，用朴素的数论知识阐述Elgamal和DSA数字签名算法.本文既能充分阐述算法所需要的数学知识，又不拘泥于细节；既能阐述其推导过程，又不过分扩展无关知识。文章最后以openssl生成的密钥作为模板，对照分析其每个参数的意义，非常适合程序员或公钥加密算法爱好者阅读。
...
### `1.3 是否一定要了解群论相关知识`    
只了解数论相关知识，而不了解群论，一样可以把Elgamal加密算法和DSA数字签名算法（DSA）阐述清楚。 当然如果了解群论，对于问题的描述更加便捷，但本文使用`集合`而非`群论`的概念来描述。   
...
到此为止，我们已经可以充分阐述`1.1`中所描述的三个问题。    
其中\\(x\\)的唯一存在性阐述如下：      
>设\\(r\\)是模正整数\\(m\\)的一个原根，由定理3.1可知，下列整数：        
$$r^1,r^2,...,r^{\varphi(m)}$$         
构成了模\\(m\\)的一个既约剩余系。因此若\\((a,m)=1\\),则存在为唯一一个整数\\(x且1 \le x \le \varphi(m)\\)使得：    
$$r^x \equiv  a \ \ mod \ m \ \ \  ( 1 \le x \le \varphi(m) )$$
特别的，当\\(m\\)是一个素数时，
\\(r^x  \ \ mod \ m \\)可以遍历\\((1,m-1)\\)集合中所有的数。

工程在用的DSA数字签名算法是Elgamal数字签名算法的扩展，涉及到`阶`的相关性质。
...
**说明** 对照上面的例子当\\(n=11\\)时\\(\varphi(11)=10\\),被10整除的数只有\\(\\{2,5,10\\}\\),所以模11的`阶`\\(ord\_{n} r=s\\)只可能为\\(\\{2,5,10\\}\\)，且每个阶为\\(s\\)的原根\\(r\\)都生成了\\(ord\_{n} r=s\\)个数,符合预期。 可以总结为  
...
数论知识介绍到这里，可以推导出后面的`Elgama签名算法`和`DSA签名算法`。
...
>由私钥\\(k\_{pr}=(p,\alpha,d)\\)很容易计算公钥 \\(k\_{pub}=(p,\alpha,\beta)\\),反之则非常困难。          
...
`4.签名和验证`   
`4.1` 首先生成私钥，并导出公钥以备签名验证使用        
>```
>生成2048bit的私钥，输出到文件 dsakey_2048.pem 
>openssl dsaparam -noout -out dsakey_2048.pem -genkey 2048    
导出公钥到文件pubkey_2048.pem          
openssl dsa -in  dsakey_2048.pem   -pubout -out pubkey_2048.pem  

 
`4.2` 用私钥签名文件 file.txt
>```
>openssl dgst -sha256 -sign dsakey_2048.pem  -out signature_2048.sign file.txt

公钥可以用来验证签名，由于私钥可以导出公钥，所以也可以用私钥验证签名。    
`4.3` 用私钥验证签名  
>```
...
实际是先计算文件的sha256,然后对此hash做签名。且每次签名都不同，因为签名的参数中有个随机数字\\(k\_E\\)    
`5.一点说明`        
DSA签名算法只能用来签名，虽然在数学上，可以利用DSA的参数进行加密，但事实上在工程上并没有此种加密标准，
所以openssl中并没有跟DSA相关的加密命令。而RSA既可以用来加密也可以用来签名。
